...
var generateMap = function(){
  ...
  navigator.geolocation.getCurrentPosition(function(position) {
    ...
    if($('#grocery-or-restaurant').is(':checked')){
      mapContainerEl.html(`<iframe width='450' height='250' frameborder='0' style='border:0' src= 'https://www.google.com/maps/embed/v1/search?key=AIzaSyBvqgE3J3k27e7smiiZAgKLa0bsGoNuHys&q=${restaurantType}+restaurant+near+${zipCode}&center=${lat},${long}&zoom=10' allowfullscreen> </iframe>`);
...
$('#grocery-or-restaurant').change(function(){
  if ($(this).is(':checked')) {
    ...
    var restaurantSelectEl = $('<select>').attr('id', 'restaurantType').attr('name', 'restaurantType');
    // create options for select element
    var americanOptionEl = $('<option>').html('American').attr('value', 'american');
    var chineseOptionEl = $('<option>').html('Chinese').attr('value', 'chinese');
    var italianOptionEl = $('<option>').html('Italian').attr('value', 'italian');
    var mexicanOptionEl = $('<option>').html('Mexican').attr('value', 'mexican');
    var optionsArr = [americanOptionEl, chineseOptionEl, italianOptionEl, mexicanOptionEl];
